...
class Queue {
  ...
  async push({ track, requestedBy = null, operator = false, block = null, intent = null, introScript = null, introKind = 'dj-speak', introPersona = null, aiPicked = false, allowDuplicate = false, linkPrev = null, linkClockAt = null }: {
    ...
    // The blocklist is absolute, so it sits above `allowDuplicate`. Every
    // playback path funnels through push(), making this the last line even for
    // sources that bypass the subsonic/library filters.
  ...
  async announce(
    ...
    if (!text || !text.trim()) return { accepted: false, deferred: false, completed: Promise.resolve(false) };
    try {
      ...
      if (placement !== 'immediate') {
        ...
        const accepted = this.holdForNextTrack(kind, [{ text, wavPath, persona, meta }], {
          exchange: false,
  ...
  holdForNextTrack(
    ...
    { exchange = false, daypart, pauseTalk = false, sfx: selectedSfx = null, onCompleted }:
      { exchange?: boolean; daypart?: string | null; pauseTalk?: boolean; sfx?: string | null; onCompleted?: (aired: boolean) => void } = {},
  ): boolean {
