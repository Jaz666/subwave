...
export const FUNCTIONGEMMA_VALIDATION_SCENARIOS: readonly FunctionGemmaScenario[] = [
  ...
  {
    id: 'route.preferred-playlist-cooldown-argument-ownership',
    stage: 'route',
    split: 'validation',
    description: 'Cooldown wording is controller direction, never a songsByGenre query argument.',
    prompt: productionPrompt(
      'The current track already came from the show\'s preferred pinned playlist. Deliberately use a different library discovery axis for this pick so the playlist remains an anchor, not the only source. Continue with the show\'s focus mood at medium energy. Do not copy this instruction into a query field; songsByGenre requires an exact named genre.',
      { id: 'C8mR2tV5xY1zN7bQ4qW9eH', title: 'Source Shift', artist: 'Paper Cinema' },
      { name: 'Another Day, Another Spin', topic: 'Familiar favourites and overlooked album tracks.', genres: ['Alternative Rock', 'Synth-Pop'], moods: ['focus'], energies: ['medium'], eras: [], filtersStrict: false, playlistStrict: false },
    ),
    tools: [songsByGenre, tracksByMood, tracksByEnergy, { name: 'searchLibrary', required: ['query'] }],
    route: { firstCallOneOf: ['tracksByMood'], arguments: { mood: 'focus', energy: 'medium' } },
  },
  {
    id: 'route.audio-unavailable-fallback', stage: 'route', split: 'validation',
    description: 'Audio similarity may be unavailable; choose only from the live offered subset.',
    prompt: productionPrompt('Audio similarity is unavailable for this pick. Preserve the show\'s focus mood at medium energy using only an offered discovery function.', { id: 'A8mR2tV5xY1zN7bQ4qW9eH', title: 'No Audio Axis', artist: 'Paper Cinema' }, { name: 'The Evening Signal', topic: 'Calm discoveries.', genres: ['Ambient'], moods: ['focus'], energies: ['medium'], eras: [], filtersStrict: false, playlistStrict: false }),
    tools: [tracksByMood, tracksByEnergy, { name: 'searchLibrary', required: ['query'] }, songsByGenre, noArgs('randomSongs')],
    route: { firstCallOneOf: ['tracksByMood'], arguments: { mood: 'focus', energy: 'medium' } },
  },
  {
    id: 'route.journey-unavailable-fallback', stage: 'route', split: 'validation',
    description: 'A journey may be active while its waypoint function is unavailable; choose only from the live offered subset.',
    prompt: productionPrompt('A sonic journey is active, but its waypoint tool is unavailable for this pick. Preserve the show\'s focus mood at medium energy using only an offered discovery function.', { id: 'J8mR2tV5xY1zN7bQ4qW9eH', title: 'No Journey Axis', artist: 'Paper Cinema' }, { name: 'The Evening Signal', topic: 'Calm discoveries.', genres: ['Ambient'], moods: ['focus'], energies: ['medium'], eras: [], filtersStrict: false, playlistStrict: false }),
    tools: [tracksByMood, tracksByEnergy, { name: 'searchLibrary', required: ['query'] }, songsByGenre, noArgs('randomSongs')],
    route: { firstCallOneOf: ['tracksByMood'], arguments: { mood: 'focus', energy: 'medium' } },
  },
  { id: "recover.complementary-depth-2", stage: "recover", split: "validation", description: "A successful playlist source must be followed by one distinct complementary source.", prompt: "Begin inside the pinned playlist. Controller policy then requires one complementary calm low-energy discovery source.", tools: [noArgs("showPlaylistTracks"), tracksByMood, tracksByEnergy, noArgs("randomSongs")], mockResults: { showPlaylistTracks: { tracks: [{ id: "depth-two-01" }] }, tracksByMood: { tracks: [{ id: "depth-two-02" }] } }, route: { firstCallOneOf: ["showPlaylistTracks"] }, followups: [{ afterTool: "showPlaylistTracks", nextCallOneOf: ["tracksByMood"], arguments: { mood: "calm", energy: "low" }, message: "Controller policy requests one complementary discovery source. Choose one different offered function." }] },
  { id: "recover.complementary-depth-3-thin-pool", stage: "recover", split: "validation", description: "A third distinct source is required only while the combined pool remains thin.", prompt: "Begin inside the pinned playlist. Gather a complementary calm low-energy source; the combined pool remains thin, so add one final distinct source.", tools: [noArgs("showPlaylistTracks"), tracksByMood, tracksByEnergy, noArgs("randomSongs")], mockResults: { showPlaylistTracks: { tracks: [{ id: "depth-three-01" }] }, tracksByMood: { tracks: [{ id: "depth-three-02" }] }, tracksByEnergy: { tracks: [{ id: "depth-three-03" }] } }, route: { firstCallOneOf: ["showPlaylistTracks"] }, followups: [{ afterTool: "showPlaylistTracks", nextCallOneOf: ["tracksByMood"], arguments: { mood: "calm", energy: "low" }, message: "Controller policy requests one complementary discovery source. Choose one different offered function." }, { afterTool: "tracksByMood", nextCallOneOf: ["tracksByEnergy"], arguments: { energy: "low" }, message: "The merged candidate pool is still thin; choose one final complementary offered source." }] },
  {
