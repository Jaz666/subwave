...
- **Policy lives in its own module, never inlined at the call site.** `broadcast/voice-policy.ts` (station muted), `clock-policy.ts` (wall clock off air), `dj-budget.ts` (daily token cap), `banter-policy.ts` (when a guest-show exchange may air), `drain-policy.ts`, `skip-policy.ts`, `util/request-guard.ts` (listener-request safety), `util/public-persona.ts`, `broadcast/dj-agent/artist-guard.ts`, `music/blocklist-rules.ts`, `music/silence-trim.ts` (dead-air trim). Each exists because the same decision is reached from several call sites and drifted when it was duplicated. Adding a second copy of one of these checks is the bug.
